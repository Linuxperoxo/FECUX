...
fecux::main::package::package(const char* _pkg_name) noexcept
  ...
  try{
    ...
    std::cerr << RED << "ERROR: " << NC << _paex.what() << " in file -> " << RED << _paex.getFile() << NC << " line -> " << RED << _paex.getLine() << NC << '\n';
    exit(EXIT_FAILURE);
  }
}
...
void fecux::main::package::load_functions(){
  ...
  if(!fecux::tools::runtime::filesUtils::is_file(*_build_file_locale)){
    fecux::utils::string _what;
    _what._cat_str("Build file was not found in -> ", RED, *_build_file_locale, NC);
    throw fecux::tools::runtime::exception(_what, BUILDPKG_FILE_NOT_FOUND);
  }

  const char* _functs_name[POSSIBLE_FUNCS] = {
    "pre_build", 
    "build", 
    "pos_build", 
    "pre_install", 
  ...
  for(int i = 0; i < POSSIBLE_FUNCS; i++){
    _command._cat_str("declare -f ", _functs_name[i], " ", *_build_file_locale, " &> /dev/null");
  ...
  if(!_found){
    fecux::utils::string _what;
    _what._cat_str("No function found in the build file ->", RED, *_build_file_locale, NC, "! The file must have at least one of these functions -> { ", RED, "pre_build, build, pos_build, pre_install, install, pos_install", NC, " }");
  ...
  make_obj<func>(_pkg_func, _enable_functs[0], _enable_functs[1], _enable_functs[2], _enable_functs[3], _enable_functs[4], _enable_functs[5]);
}
