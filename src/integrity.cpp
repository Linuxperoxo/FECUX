...
char **fecux::tools::runtime::integrity::read_file(const char **_names,
  ...
  for (size_t i = 0; i < _element_size; i++) {
    if (!_libconfig->lookupValue(_names[i], _buffer) || _buffer.empty()) {
      ...
      _what._cat_str("Unable to load the variable -> ", RED, _names[i], NC,
                     " in file -> ", RED, _file, NC);
...
void fecux::tools::runtime::integrity::verify_dirs(
    const fecux::main::config *_config) noexcept {

  const char *_dirs[NUM_CHECK_DIRS] = {
      _config->source_dir(), _config->fakeroot_dir(), MAIN_REPO_DIR, WORLD_DIR};
  ...
  for (int i = 0; i < NUM_CHECK_DIRS; i++) {
    if (!fecux::tools::runtime::filesUtils::is_dir(_dirs[i])) {
      ...
      std::cerr << YELLOW << "::: " << NC << GREEN << _dirs[i] << NC
                << " Created! :)\n";
...
char **fecux::tools::runtime::integrity::verify_config() {
  const char *_options_name[OPTS_NUM] = {"source_dir", "fakeroot_dir", "cflags",
                                         "cxxflags", "jobs"};
